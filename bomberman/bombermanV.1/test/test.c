...
void handle_json(struct Client client, char *buff)
{
    ...
    if (compare(token[0], "GET") == 0)
    {
        ...
        char *gege = "maps/list";
        printf("%ld\n", sizeof(gege));
        if (compare(token[1], "maps/list") == 0)
        {
        ...
        if (compare(token[1], "game/list") == 0)
        {
...
void *handle_client(void *socket_desc)
{
    ...
    if ((size = read(socket_client, buf, sizeof(buf) - 1)) < 0)
    {
        perror("Erreur de lecture du pseudo");
        close(socket_client);
        return NULL; // Ajout pour éviter l'exécution du reste de la fonction en cas d'erreur
    }
    ...
    while ((size = read(socket_client, buffer, sizeof(buffer) - 1)) > 0)
    {
        buffer[size] = '\0';
        handle_json(*client, buffer);
    }
